...
| WinGet | `winget upgrade --id EllipseSoftware.Bast --exact` |
| Source build | Link back to bast.sh |

Press `v` in the TUI to see version info and the update suggestion.

WinGet releases require review in the community repository. Bast may report a newer GitHub release before it is available through `winget upgrade`.
