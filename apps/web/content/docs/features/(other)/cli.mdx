...
Sync imports cloud hosts as read-only. Cloud connections through `bast connect` or `bast <label>` use the same banner and access-preparation flow as the TUI. See [GCP](/docs/features/gcp), [AWS](/docs/features/aws), [Azure](/docs/features/azure), [DigitalOcean](/docs/features/digitalocean), [box.ascii.dev](/docs/features/box), or [Upstash Box](/docs/features/upstash) for provider setup.

```bash
bast digitalocean new <name> [--region nyc3] [--size s-1vcpu-1gb] [--image ubuntu-24-04-x64]
