...
Sync imports cloud VMs as read-only hosts. Cloud connections through `bast connect` or `bast <label>` use the same banner and access-preparation flow as the TUI. See [GCP](/docs/features/gcp), [AWS](/docs/features/aws), or [Azure](/docs/features/azure) for provider setup and routing.
