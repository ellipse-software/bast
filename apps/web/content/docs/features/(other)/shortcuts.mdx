...
Press `?` inside Bast for the in-app reference. Press `v` for version info, credits, and update hints. A first TUI with no hosts is a skippable start chooser; see [First launch](/docs/features/onboarding).
