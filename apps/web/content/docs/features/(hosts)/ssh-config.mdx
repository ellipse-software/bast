...
Presentation metadata (groups, tags, colors, notes, favorites, recency) lives in `~/.config/bast/state.json`. Connection settings live in SSH config files where they belong. Host passwords, if you save them, live in `~/.config/bast/passwords/` and are never written to SSH config.
