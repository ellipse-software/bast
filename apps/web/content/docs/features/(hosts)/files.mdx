...
SFTP uses `BatchMode` for key-based hosts, so passphrase and host-key prompts cannot interrupt the TUI. Hosts with a stored password use askpass instead. If connect fails, unlock the key with `ssh-add`, or Connect once from Hosts to accept the host key, then retry.
