...
Hosts imported from [GCP](/docs/features/gcp), [AWS](/docs/features/aws), [Azure](/docs/features/azure), [box.ascii.dev](/docs/features/box), [Upstash Box](/docs/features/upstash), or [Railway](/docs/features/railway) are read-only for connection settings and grouping. GCP, AWS, and Azure hosts can still be favorited or hidden; Box, Upstash, and Railway hosts are fully sync-owned. Disconnect the provider in the Sync tab to remove them. Bast prepares provider-specific SSH access immediately before connecting.
