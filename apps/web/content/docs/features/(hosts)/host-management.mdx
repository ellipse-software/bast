...
Hosts imported from [GCP](/docs/features/gcp), [AWS](/docs/features/aws), [Azure](/docs/features/azure), [DigitalOcean](/docs/features/digitalocean), [box.ascii.dev](/docs/features/box), or [Upstash Box](/docs/features/upstash) are read-only for connection settings and grouping. GCP, AWS, Azure, and DigitalOcean hosts can still be favorited or hidden; Box and Upstash hosts are fully sync-owned. DigitalOcean, Box, and Upstash also support create, stop, resume, and fork from Bast. Disconnect the provider in the Sync tab to remove the generated hosts. Bast prepares provider-specific SSH access immediately before connecting.
