...
Vault syncs **Bast-managed** hosts, keys, and metadata between your machines. It is separate from cloud provider sync (GCP / AWS / Azure / box.ascii.dev / Upstash Box / Railway), which lives on the **Sync** tab (`4`).
...
| Cloud integration *settings* (filters, auto-sync flags) | Cloud credentials, SA JSON paths, Upstash API keys, last-sync errors |
