...
Paused boxes stay in the **Upstash** group and are hidden from the host list until you press `.`. The group appears on Hosts once a box is visible; create from the Sync tab when it is empty. Connection settings, group, and label are owned by sync.
