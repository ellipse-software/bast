...
Paused boxes stay in the **Upstash** group and are hidden from the host list until you press `.`. If none are running, the **Upstash** group is omitted from Hosts until `.`. Connection settings, group, and label are owned by sync.
