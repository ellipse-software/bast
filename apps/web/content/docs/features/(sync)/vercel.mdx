...
A project ID is optional for sync. Bast first lists sandboxes for the whole team. If Vercel rejects that, set one or more project IDs (`prj_…` or names, comma-separated). Creating a sandbox still needs a project.
...
3. Choose **Connect**. Paste the token and team ID. Project is optional.
...
```bash
bast vercel token --team team_xxx
bast vercel token --team team_xxx --project prj_xxx
bast vercel token --team team_xxx --project prj_app,prj_api
bast sync vercel
bast sync status
bast sync disable vercel
```

If a token and team are already stored, Bast **auto-connects** on TUI start and `bast sync status`, unless you previously chose **Disconnect**.
