...
Hobby sandboxes time out after 45 minutes. Pro/Enterprise can run up to 5 hours while connected.
...
Stopped sandboxes stay in the **Vercel** group and are hidden from the host list until you press `.`.
...
Press `r` on the Sync tab (or `bast sync vercel`) after you create, stop, or delete sandboxes outside Bast. Disconnecting Vercel removes the generated sync config. The token file stays until you delete `~/.config/bast/vercel-token`.
