...
- The team ID (`team_…`)
- At least one project ID or name (`prj_…`)
...
3. Choose **Connect**. Paste the token, team ID, and a project ID.

After connect, **Add project** and **Remove project** import extra Vercel projects the same way Hetzner tokens work: one chip to add, a picker to remove.

Bast writes the token to `~/.config/bast/vercel-token` (mode `0600`). Team and project IDs stay in `state.json`. The token is not stored in SSH config or Vault. Disconnect leaves the token file in place.

You can also set `VERCEL_TOKEN`, `VERCEL_TEAM_ID`, and `VERCEL_PROJECT_ID`. `VERCEL_PROJECT_ID` accepts several IDs separated by commas. The first successful sync copies the token into the local file.

CLI:

```bash
bast vercel token --team team_xxx --project prj_xxx
bast vercel token --project prj_api
bast vercel token --remove prj_api
bast sync vercel
bast sync status
...
```bash
bast vercel new [--name name] [--project id] [--vcpus 1|2|4] [--timeout 15m|1h|5h] [--ephemeral]
