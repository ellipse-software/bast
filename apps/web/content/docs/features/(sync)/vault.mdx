...
On first link, local and remote state are merged by stable IDs (`ManagedID` for hosts, key fingerprint for keys). True conflicts (same alias, different managed IDs) open a chooser in the TUI: keep this machine, keep remote, or cancel. From the CLI:
