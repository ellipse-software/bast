...
Ciphertext is stored by Bast. The encryption passphrase never leaves your machine. Bast keeps a local copy in `~/.config/bast/vault-passphrase` (mode `0600`) so linked machines stay unlocked across launches. Log out deletes it.
...
In the TUI, linked machines pull in the background after local hosts load when a local passphrase is available. Local edits debounce a push after a few seconds. Opening Bast never waits on the network to show your hosts.
