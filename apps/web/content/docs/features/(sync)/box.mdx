...
Stopping a box snapshots it and pauses billing (`archiving` → `archived`). That can take a few minutes. Stopped and stopping boxes drop off the visible host list (press `.` to show them) and stay under **Box** so you can resume later. If no boxes are running, the **Box** group is hidden until you press `.`.
