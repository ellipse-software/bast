...
The **Sync** tab (`3`) imports [ASCII Box](https://box.ascii.dev/) sandboxes into Bast. Synced boxes show up in the host picker under **Box**, stay connectable with your system `ssh`, and keep connection details owned by Box sync.
...
Stopping a box snapshots it and pauses billing (`archiving` → `archived`). That can take a few minutes. Bast keeps the host visible and muted while the box is stopping, then keeps it under **Box** so you can resume later.
