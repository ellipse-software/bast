...
Running boxes show **Box synced**. Stopped or archiving boxes stay in the same group but are hidden from the host list until you press `.` (the same toggle that shows user-hidden hosts). Search still finds them by name. If none are running, the **Box** group is omitted from Hosts until `.`. Selecting the **Box** group shows running vs stopped counts and **New box**. Connection settings, group, and label are owned by sync, so edit (`e`), delete (`d`), move, favorite, and hide are blocked.
