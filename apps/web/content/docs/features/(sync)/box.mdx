...
Bast supports **GCP**, **AWS**, **Azure**, **box.ascii.dev**, and **Fly.io** through each provider's native CLI, and **Upstash Box** through its API.
...
Running boxes show **Box synced**. Stopped or archiving boxes stay in the same group but are hidden from the host list until you press `.` (the same toggle that shows user-hidden hosts). Search still finds them by name. The **Box** group appears on Hosts once a box is visible; create from the Sync tab when the group is empty. Selecting the group shows running vs stopped counts and **New box**. Connection settings, group, and label are owned by sync, so edit (`e`), delete (`d`), move, favorite, and hide are blocked.
