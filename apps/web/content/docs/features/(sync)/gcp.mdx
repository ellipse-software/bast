...
The **Sync** tab (`4`) pulls servers from cloud providers into Bast. Synced hosts show up in the host picker under a provider group, stay connectable with your system `ssh`, and keep connection details owned by the provider. You do not edit them like Bast-managed hosts.
...
The GCP provider page on Sync (`4`) lists those instances in a collapsible **Instances** group. Enter connects.
