...
| Key | Action |
| --- | --- |
| `g` | Generate a new ed25519 or RSA keypair |
| `i` | Import an existing private key from a file or pasted PEM |
| `e` | Edit the comment on a public key |
| `a` | Push a public key to a remote `~/.ssh/authorized_keys` |
| `x` | Export key files to a folder you choose |
| `p` | Promote an external key into Bast-managed storage, or change a managed key passphrase |
...
Copy an external key from `~/.ssh` into Bast-managed keys so it can participate in [Vault](/docs/features/vault):

```bash
bast keys promote id_ed25519 --name work
```
