...
export const cloudSshGuide: GuidePage = {
  ...
  description:
    "Import live GCP, AWS, and Azure VMs plus box.ascii.dev, Upstash Box, and Railway services into Bast as read-only SSH hosts. Keep OpenSSH as the connection path, and stop maintaining cloud host spreadsheets.",
  keywords: [
    "GCP SSH",
    "AWS SSH",
    "Azure SSH",
  ...
  lead: "Cloud consoles and one-off CLI copy-paste are fine until you have dozens of hosts. Bast imports live inventory from GCP, AWS, Azure, box.ascii.dev, Upstash Box, and Railway, then connects with normal OpenSSH.",
  ...
  steps: [
    <>
      ...
      Upstash Box API key with <Code>bast upstash key</Code>, or a Railway
      account token with <Code>bast railway key</Code>.
    </>,
    <>
      Run <Code>bast sync gcp</Code>, <Code>bast sync aws</Code>,{" "}
      <Code>bast sync azure</Code>, <Code>bast sync box</Code>,{" "}
      <Code>bast sync upstash</Code>, or <Code>bast sync railway</Code> (or use
    ...
    <>
      ...
      <DocLink href="/docs/features/railway">Railway</DocLink> guide for
      provider notes.
    </>,
  ],
  sections: [
    {
      ...
      paragraphs: [
        <>
          ...
          Railway store API tokens in local 0600 files, not in SSH config or
          Vault.
        </>,
      ],
    },
