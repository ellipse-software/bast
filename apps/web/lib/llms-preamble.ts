...
- Import live GCP, AWS, Azure, box.ascii.dev, Upstash Box, or Railway inventory as read-only hosts
...
How to call Bast.sh: install the CLI (\`brew install ellipse-software/tap/bast\` or \`curl -fsSL ${siteUrl}/install | sh\`), then run \`bast --json\` commands. For coding agents, also install the skill with \`npx skills add ellipse-software/bast -g -y\` and read this file plus ${siteUrl}/openapi.json for the hosted HTTP API (Vault, health, telemetry).
...
- [Bast.sh CLI](${siteUrl}/cli): Official CLI via Homebrew, WinGet, and install scripts
