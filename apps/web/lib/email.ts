...
function getEmailClient() {
	...
	if (!apiToken || !accountId) {
		throw new Error(
			"CLOUDFLARE_API_TOKEN and CLOUDFLARE_ACCOUNT_ID (or R2_ACCOUNT_ID) are required",
		);
	}
...
export async function sendVaultOTP(to: string, otp: string): Promise<void> {
	...
	if (!emailConfigured()) {
		...
		throw new Error(
			"CLOUDFLARE_API_TOKEN and CLOUDFLARE_ACCOUNT_ID are required to send vault OTP email",
		);
	}
