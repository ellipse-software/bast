...
Bast reads existing SSH configuration, stores its managed hosts in included OpenSSH files, and launches the system `ssh` binary for connections. Vercel Sandboxes are the exception: connect uses a WebSocket PTY, not OpenSSH. It also manages SSH keys, imports hosts from shell history, and can sync hosts from GCP, AWS, Azure, [box.ascii.dev](https://bast.sh/docs/features/box), [Upstash Box](https://bast.sh/docs/features/upstash), [Vercel Sandbox](https://bast.sh/docs/features/vercel), and [Hetzner Cloud](https://bast.sh/docs/features/hetzner). See the [Bast documentation](https://bast.sh/docs) for installation, usage, and feature guides.
