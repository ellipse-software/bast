...
Bast reads existing SSH configuration, stores its managed hosts in included OpenSSH files, and launches the system `ssh` binary for connections. It also manages SSH keys, imports hosts from shell history, and can sync hosts from GCP, AWS, Azure, [box.ascii.dev](https://bast.sh/docs/features/box), [Upstash Box](https://bast.sh/docs/features/upstash), and [Railway](https://bast.sh/docs/features/railway). See the [Bast documentation](https://bast.sh/docs) for installation, usage, and feature guides.
