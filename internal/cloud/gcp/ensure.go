...
// EnsureConfig controls gcloud-style SSH access ensure for a synced instance.
type EnsureConfig struct {
	...
	Status          func(string)  // optional progress messages for interactive connects
}

// EnsureResult is the SSH auth Bast should use after ensure.
...
// ParseSyncID extracts project/zone/name from a Bast GCP sync ID.
...
// EnsureAccess prepares SSH auth for a synced instance.
// Prefer a local key already authorized on the VM; otherwise ensure the
// gcloud identity (~/.ssh/google_compute_engine) and publish it when needed.
